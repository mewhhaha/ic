...
This case study compiles a small pure Duck synthesizer through gpufuck and
...
checks that the result is `Bytes`, and writes the file only when run as a
command. The renderer itself performs no I/O, which lets the tests inspect the
returned `Uint8Array`.
...
```sh
deno test --allow-read case-studies/wav/wav.test.ts
```
